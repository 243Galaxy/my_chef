<div>
    <div class="p-4 sm:ml-64 relative overflow-x-auto shadow-md sm:rounded-lg mt-12">
        ...
        @endif
        <a href="{{ route('admin.new.food') }}" class="mt-4 px-4 py-2 hover:bg-blue-600 dark:hover:bg-blue-600 text-white rounded-md bg-blue-500 dark:bg-blue-500">New</a>
                    ...
                    <th scope="row" class="flex items-center px-2 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                        <div class="ps-3">
                            <div class="text-base font-semibold">
                                <img src="{{ asset('imgs/uploads') }}/{{ $food->picture }}" width="50" alt="Logo">
                                {{ $food->name }}
                            </div>
                            <div class="font-normal text-gray-500">{{ $food->other }}</div>
                            <div class="font-normal text-gray-500"> Category: {{ $food->category->name ?? 'No Category' }}</div>
                        </div>  
                    </th>
                    <td class="px-6 py-4">
                        <p>
                          ...
                          <a href="{{ route('admin.new.recipe', ['food_id'=>$food->id]) }}" class="font-bold text-white dark:text-white hover:bg-green-700 bg-green-600 rounded-md p-2 text-xs">Add recipe</a>
                          <a href="{{ route('admin.view.recipe', ['food_id'=>$food->id]) }}" class="font-bold text-white dark:text-white hover:bg-blue-700 bg-blue-600 rounded-md p-2 text-xs">Show recipe</a>
                    </td>
                    <td class="px-6 py-2 flex">
                        <a href="{{ route('admin.edit.food',['food_id'=>$food->id]) }}" class="font-medium text-white dark:text-white bg-orange-500 rounded-md p-2 text-sm mx-2">Edit</a>
                        <!-- Modal toggle -->
                        <button id="deleteButton" wire:click="deleteId({{ $food->id }})" data-modal-target="deleteModal" data-modal-toggle="deleteModal" class="text-white bg-red-700 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 rounded-lg text-sm p-2 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" type="button" wire:click="">
                            Delete
                        </button>
                    </td>
                </tr>
