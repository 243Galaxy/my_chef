            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mt-2">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="w-8 p-4">
                            <div class="ps-3">
                                <img src="{{ asset('imgs/uploads') }}/{{ $recipe->picture }}" width="100" class="rounded-md" alt="Recipe">
                                <div class="text-base font-semibold ml-8 mt-2">{{ $recipe->name }}</div>
                                <div class="text-base font-semibold"> <b>Measure:</b> {{ $recipe->measure }}</div>
                            </div>  
                        </td>
                        <td class="px-6 py-2 flex">
                            <a href="{{ route('admin.edit.recipe',['recipe_id'=>$recipe->id]) }}" class="font-medium text-white dark:text-white hover:underline bg-orange-500 rounded-md p-2 text-sm mx-2">Edit</a>
                            <!-- Modal toggle -->
                            <button id="deleteButton" wire:click="deleteId({{ $recipe->id }})" data-modal-target="deleteModal" data-modal-toggle="deleteModal" class="text-white bg-red-700 hover:bg-red-800 focus:ring-2 focus:outline-none focus:ring-red-300 rounded-lg text-sm p-2 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" type="button" wire:click="">
                                Delete
                            </button>
                        </td>
                    </tr>
